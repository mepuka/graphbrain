"""PostgreSQL repository for classification entities.

Provides CRUD operations for semantic classes, predicate banks,
patterns, and classifications. Uses the same PostgreSQL connection
as the hypergraph backend.
"""
...
class ClassificationRepository:
    ...
    Usage:
        ...
        repo = ClassificationRepository(connection_string)
        ...
        repo.save_predicate(PredicateBankEntry(class_id=sem_class.id, lemma="claim"))

        # Query
        predicates = list(repo.get_predicates_by_class(sem_class.id))
    """
    ...
    def get_all_patterns(self) -> Iterator[tuple[str, list[ClassPattern]]]:
        ...
        Yields:
            Tuples of (class_id, list[ClassPattern]) for each class with patterns.
        """
        with self._conn.cursor() as cur:
            cur.execute(
                """
